...
bool AstarteGenericProducer::sendData(const QList<QVariant> &value, const QByteArray &target, const QDateTime &timestamp, const QVariantHash &metadata)
{
    ...
    for (it = m_mappingToTokens.constBegin(); it != m_mappingToTokens.constEnd(); it++) {
        ...
        QByteArray matchedMapping = it.key();
        qWarning() << "expected " << m_mappingToArrayType.value(matchedMapping) << "got" << value[0].type();

        if (!value[0].canConvert(m_mappingToArrayType.value(matchedMapping))) {
            qWarning() << "Invalid type for value in sendDataOnEndpoint, expected " << m_mappingToArrayType.value(matchedMapping) << "got" << value[0].type();
            return false;
        }
        qWarning() << "- - expected " << m_mappingToArrayType.value(matchedMapping) << "got" << value[0].type();
